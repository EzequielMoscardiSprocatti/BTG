using BTG.Api.Configs;
using BTG.Api.Extensions;
using BTG.Aplicacao.Comandos.Pedido;
using BTG.Aplicacao.Consumers;
using BTG.Aplicacao.Dtos;
using BTG.Dominio.Contratos;
using BTG.Infra.Contextos;
using BTG.Infra.Repositorios;
using MassTransit;
    ...
    options.AddPolicy("PoliticaPadrao", policy =>
    {
        policy.SetIsOriginAllowed(origin =>
        {
            return Dominios.Any(dominio => Regex.IsMatch(origin, dominio))
                || DominiosDev.Any(dominio => Regex.IsMatch(origin, dominio));
        })
        .AllowAnyHeader()
        .AllowAnyMethod()
        .AllowCredentials();
